...
-----------------------------------------------*/
ENT.Model = "models/gredwitch/m2browning/m2_tripod.mdl"
ENT.StartHealth = 200
...
function ENT:Init()
	...
	gunModel:SetModel("models/gredwitch/m2browning/m2_gun_anotherone.mdl")
	gunModel:SetLocalPos(self:GetPos() + self:GetUp()*50)
		...
		if self.Emp_CurrentAmmo <= 0 && self.Emp_Reloading == false then
			...
			timer.Simple(0.3, function()
				if IsValid(self) then
					...
					reloadBox:SetModel("models/gredwitch/m2browning/m2_mag.mdl")
					...
					if IsValid(phys) then
						phys:Wake()
						phys:ApplyForceCenter(reloadBox:GetForward() * -30000)
					end
