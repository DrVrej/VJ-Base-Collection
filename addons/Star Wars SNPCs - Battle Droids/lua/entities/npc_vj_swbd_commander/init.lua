...
-----------------------------------------------*/
ENT.Model = "models/vj_starwars/droid/commanderb1.mdl"
ENT.StartHealth = 120
...
function ENT:HandleGibOnDeath(dmginfo, hitgroup)
	local doGib = false
	self.DeathCorpseModel = {"models/hfg/starwars/droids/sw_bdroid_commander/torso_bg.mdl"}
	if self:IsGibDamage(dmginfo:GetDamageType()) then
		doGib = true
		self.SW_Droid_All = true
		self.DeathCorpseModel = {"models/hfg/starwars/droids/sw_bdroid_commander/torso.mdl"}
	end
	
	if hitgroup == HITGROUP_HEAD or self.SW_Droid_All then
		doGib = true
		self.SW_Droid_Head = true
		self:CreateGibEntity("prop_ragdoll", "models/hfg/starwars/droids/sw_bdroid_commander/head.mdl", {Pos=self:GetPos(), Ang=self:GetAngles(), Vel="UseDamageForce", NoFade=true, RemoveOnCorpseDelete=true}, function(gib) gib:GetPhysicsObject():SetMass(80) end)
	end
	if hitgroup == HITGROUP_RIGHTARM or self.SW_Droid_All then
		doGib = true
		self.SW_Droid_RHand = true
		self:CreateGibEntity("prop_ragdoll", "models/hfg/starwars/droids/sw_bdroid_commander/right_arm.mdl", {Pos=self:GetPos(), Ang=self:GetAngles(), Vel="UseDamageForce", NoFade=true, RemoveOnCorpseDelete=true}, function(gib) gib:GetPhysicsObject():SetMass(80) end)
	end
	if hitgroup == HITGROUP_LEFTARM or self.SW_Droid_All then
		doGib = true
		self.SW_Droid_LHand = true
		self:CreateGibEntity("prop_ragdoll", "models/hfg/starwars/droids/sw_bdroid_commander/left_arm.mdl", {Pos=self:GetPos(), Ang=self:GetAngles(), Vel="UseDamageForce", NoFade=true, RemoveOnCorpseDelete=true}, function(gib) gib:GetPhysicsObject():SetMass(80) end)
	end
	if hitgroup == HITGROUP_RIGHTLEG or self.SW_Droid_All then
		doGib = true
		self.SW_Droid_RLeg = true
		self:CreateGibEntity("prop_ragdoll", "models/hfg/starwars/droids/sw_bdroid_commander/right_leg.mdl", {Pos=self:GetPos(), Ang=self:GetAngles(), Vel="UseDamageForce", NoFade=true, RemoveOnCorpseDelete=true}, function(gib) gib:GetPhysicsObject():SetMass(80) end)
	end
	if hitgroup == HITGROUP_LEFTLEG or self.SW_Droid_All then
		doGib = true
		self.SW_Droid_LLeg = true
		self:CreateGibEntity("prop_ragdoll", "models/hfg/starwars/droids/sw_bdroid_commander/left_leg.mdl", {Pos=self:GetPos(), Ang=self:GetAngles(), Vel="UseDamageForce", NoFade=true, RemoveOnCorpseDelete=true}, function(gib) gib:GetPhysicsObject():SetMass(80) end)
	end
	if doGib then
